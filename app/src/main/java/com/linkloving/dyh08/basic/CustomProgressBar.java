package com.linkloving.dyh08.basic;
...
public class CustomProgressBar extends View {
    ...
    private int mBorderWidth; //进度条外圈宽度
    private boolean mIsShowDesc ;

    private int DEFAULT_MAX = 100;
    private int DEFAULT_PROGRESS = 0;
    private int DEFAULT_RADIUS = (int) TypedValue.applyDimension( TypedValue.COMPLEX_UNIT_DIP, 3, getResources().getDisplayMetrics());
    private int DEFAULT_PROGRESS_COLOR ;//progress的颜色
    private int DEFAULT_PROGRESS_DESC_COLOR = Color.WHITE; //progress里面的描述的颜色
    ...
    private boolean DEFAULT_ISSHOWDESC = false ; //progress里面的描述是否显示
    ...
    public CustomProgressBar(Context context, AttributeSet attrs,int defStyleAttr) {
        ...
        this.context = context;
        DEFAULT_PROGRESS_COLOR =context.getResources().getColor(R.color.progress_color);
        ...
        try {
            ...
            mBorderWidth = (int) a.getDimension(R.styleable.CustomProgressBar_ProgressBar_borderWidth,DEFAULT_BORDER_WIDTH);
...
//        mPaint.setShadowLayer(1,1,1,Color.WHITE);
        mPaint.setColor(context.getResources().getColor(R.color.progress_color_gray));
    ...
    private void drawProgress(Canvas canvas) {
        ...
        LinearGradient lg=new LinearGradient(0 , 0,  mWidth , mHeight,
                context.getResources().getColor(R.color.orangeStart),//开始颜色
                context.getResources().getColor(R.color.orangeOver),//结束颜色
        ...
        if (scale <= 1) {
            ...
            path.arcTo(new RectF(left, bottom - 2 * mRadius, left + 2 * mRadius, bottom), 180 - (float) (angle * 180 / Math.PI),(float) (angle * 180 / Math.PI));
            path.close();
            canvas.drawPath(path, mPaint);
        } else if (scale2 <= 1) {
            path.arcTo(new RectF(left, top, left + 2 * mRadius, top + 2 * mRadius), 180, 90);
            ...
            path.arcTo(new RectF(left, top, left + 2 * mRadius, top + 2 * mRadius), 180, 90);
            path.lineTo(right - mRadius, top);
            path.arcTo(new RectF(right - 2 * mRadius, top, right, top + 2 * mRadius), -90, (float) (angle * 180 / Math.PI));
            double y = Math.pow((Math.pow(mRadius, 2) - Math.pow(a, 2)), .5) + top + mRadius;
            path.lineTo(right - mRadius + a, (float) y);
            path.arcTo(new RectF(right - 2 * mRadius, bottom - 2 * mRadius, right, bottom), (float) (90 - (angle * 180 / Math.PI)), (float) (angle * 180 / Math.PI));
            path.lineTo(left + mRadius, bottom);
            path.arcTo(new RectF(left, bottom - 2 * mRadius,left + 2 * mRadius, bottom), 90, 90);
            path.close();
    ...
    private void drawProgressDesc(Canvas canvas) {
        String finalProgressDesc = mProgressDesc + "%";
        mTextPaint.getTextBounds(finalProgressDesc, 0,finalProgressDesc.length(), mTextBounds);
        canvas.drawText(finalProgressDesc,
                (int) (mWidth / 2.0 - mTextBounds.width() / 2.0),
    ...
    public void setCurProgress (final int progress) {
        this.mProgressDesc = String.valueOf(progress);
