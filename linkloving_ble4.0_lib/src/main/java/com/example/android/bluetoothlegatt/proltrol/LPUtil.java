		...
		while (buffer.remaining() > 2) {
			...
			switch (type) {
				...
				case 0x14: // List of 16-bit Service Solicitation UUIDs
					while (length >= 2) {
	/*					parsedAd.uuidStrings.add(UUID.fromString(String.format(
								"%08x-0000-1000-8000-00805f9b34fb", buffer.getShort())));*/
						parsedAd.uuidStrings.add(String.format("%x",buffer.getShort()));
						length -= 2;
					}
					break;
...
//						parsedAd.uuidStrings.add(new UUID(msb, lsb));
						length -= 16;
					}
					break;
